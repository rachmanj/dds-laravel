...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('invoices', function (Blueprint $table) {
            ...
            // Composite unique constraint: supplier_id + invoice_number
            // This prevents duplicate invoice numbers for the same supplier
            // but allows different suppliers to use the same invoice numbers
            $table->unique(['supplier_id', 'invoice_number'], 'invoices_supplier_invoice_unique');

            // Add indexes for better performance
            $table->index(['supplier_id', 'invoice_number']);
            $table->index(['status', 'created_at']);
            $table->index(['cur_loc', 'created_at']);
        });
    }
