...
<head>
    ...
    <title>Invoice Transmittal Advice - {{ $distribution->distribution_number }}</title>

    <!-- Google Font: Source Sans Pro -->
    <link rel="stylesheet"
        href="{{ asset('adminlte/fontgoogle.css') }}">
    <!-- Font Awesome -->
                ...
                <div class="col-12">
                    <table class="table">
                        <tr>
                            ...
                            <td rowspan="2">
                                <div class="document-title">Invoice Transmittal Advice</div>
                    ...
                    <table class="documents-table">
                        ...
                        <tbody>
                            ...
                            @foreach ($invoiceDocuments as $index => $doc)
                                ...
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td><strong>{{ $invoice->type->type_name ?? 'Invoice' }}</strong></td>
                                    ...
                                    @foreach ($invoice->additionalDocuments as $addDoc)
                                        <tr class="additional-doc-row">
                                            ...
                                            <td colspan="7" style="padding-left: 20px;">
                                                <em>Attached: {{ $addDoc->type->type_name ?? 'Document' }} -
                                                    {{ $addDoc->document_number ?? 'N/A' }}</em>
                                                ({{ $addDoc->document_date ? \Carbon\Carbon::parse($addDoc->document_date)->setTimezone('Asia/Singapore')->format('d-M-Y') : 'N/A' }})
                                                @if ($addDoc->po_no)
                                                    - PO: {{ $addDoc->po_no }}
                                                @endif
                                            </td>
