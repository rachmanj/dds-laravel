    ...
    <section class="content">
        <div class="container-fluid">
            ...
            <div class="card card-warning">
                <form method="post" action="{{ route('solar-price-histories.update', $solarPriceHistory) }}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="invoice_id" id="invoice_id" required>
                                ...
                                @foreach ($invoices as $inv)
                                    <option value="{{ $inv->id }}" @selected(old('invoice_id', $solarPriceHistory->invoice_id) == $inv->id)>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" inputmode="decimal" class="form-control" name="unit_price" id="unit_price"
                                value="{{ old('unit_price', $solarPriceHistory->unit_price) }}" required>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" name="period_start" id="period_start"
                                value="{{ old('period_start', $solarPriceHistory->period_start?->toDateString()) }}"
                                required>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" name="period_end" id="period_end"
                                value="{{ old('period_end', $solarPriceHistory->period_end?->toDateString()) }}" required>
                        </div>
                        <div class="form-group">
                            <label for="quantity">Quantity (optional)</label>
                            <input type="text" class="form-control" name="quantity" id="quantity"
                                value="{{ old('quantity', $solarPriceHistory->quantity) }}">
                        </div>
                        <div class="form-group">
                            <label for="amount">Amount (optional)</label>
                            <input type="text" class="form-control" name="amount" id="amount"
                                value="{{ old('amount', $solarPriceHistory->amount) }}">
                        </div>
                        <div class="form-group">
                            <label for="notes">Notes</label>
                            <textarea class="form-control" name="notes" id="notes" rows="2">{{ old('notes', $solarPriceHistory->notes) }}</textarea>
                        </div>
                    </div>
...
@section('scripts')
    <script>
        (function() {
            ...
            const selectedLine = @json(old('invoice_line_detail_id', $solarPriceHistory->invoice_line_detail_id));
                ...
                fetch(base + '/' + invoiceId + '/solar-lines', {
                    ...
                    .then(data => {
                        ...
                        (data.lines || []).forEach(function(l) {
                            ...
                            if (String(selectedLine) === String(l.id)) opt.selected = true;
