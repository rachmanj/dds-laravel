...
**Stack**: Laravel 12, Blade, AdminLTE 3 / Bootstrap 4, jQuery, Spatie Laravel Permission (`can`, `hasAnyRole`), session/web auth.

**Implemented in this repository (DDS Laravel)**: Yes — see [Navbar Menu Search](architecture.md#navbar-menu-search) in [`docs/architecture.md`](architecture.md), the [2026-05-10 decision](decisions.md#decision-navbar-menu-search-2026) in [`docs/decisions.md`](decisions.md), and [`tests/Feature/MenuSearchTest.php`](../tests/Feature/MenuSearchTest.php). The file list below matches this app.
...
- **DDS Laravel**: the script reads **`/api/menu/search`** from **`<meta name="menu-search-url" content="{{ route('api.menu.search') }}">`** in `head.blade.php`, with a fallback to **`/api/menu/search`**. For other deployments, use the same pattern if routes are prefixed or behind a non-root URL.
...
- Architecture: [architecture.md](architecture.md) — **Navbar Menu Search** (implemented 2026-05-10).
- Decision record: [decisions.md](decisions.md) — **2026-05-10 — Navbar menu search**.

---

## License / reuse

This reference is written for portability; **DDS Laravel** implements it as documented above. For other applications, adapt file paths, permission names, and routes; no external dependencies beyond Laravel, jQuery, and (optional) Spatie Permission are required for the same pattern.
