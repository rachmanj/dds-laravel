...
-   Any workflow `status` **except** `cancel` may be synced (`open`, `verify`, `return`, `sap`, `close`); on successful post, workflow `status` is set to `sap` automatically
-   Invoices with legacy manual `sap_doc` and `sap_status = null` (**`has_legacy_sap_doc`**) cannot be synced — prevents duplicate AP Invoices in SAP B1
-   **Sync is manual only** - user must click "Send to SAP" button
-   `sap_status` transitions: null → pending → posted/failed (also `cancelling` → `cancelled` for AP cancellation)
-   Failed invoices can be retried (reset to pending) unless blocked by legacy `sap_doc`
-   **DocNum is auto-generated by SAP** - stored in `sap_doc_num` after successful sync (also copied to `sap_doc` when unique)
...
-   ✅ Workflow `status` is not `cancel` (open/verify/return/sap/close allowed)
-   ✅ Not a legacy manual `sap_doc` row (`has_legacy_sap_doc` = false)
-   ✅ Supplier has `sap_code`
-   ✅ Supplier exists in SAP (CardType = 'S' or 'C')
-   ✅ Invoice amount > 0
-   ✅ Invoice date is valid
-   ✅ Currency code is valid SAP currency
-   ✅ `sap_status` not in `pending`, `posted`, or `cancelling`
-   ⚠️ Project codes exist in SAP (if provided)
-   ⚠️ Tax code is valid (if custom tax logic)
-   ⚠️ GRPO line references when `po_no` is set
...
-   **"Send to SAP" button** (only if eligible: `status !== 'cancel'`, not `has_legacy_sap_doc`, and `sap_status` is null, `failed`, or `cancelled`)
-   **"Retry SAP Sync" button** (if failed and not legacy `sap_doc`)
...
-   SAP status column with badge ✅ (2026-07-28)
-   Filter by SAP status (pending, posted, failed, not_sent) — deferred
